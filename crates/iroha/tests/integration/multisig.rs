...
fn multisig_recursion() -> Result<()> {
    ...
    let instructions = vec![Register::domain(Domain::new(domain_id.clone())).into()];
    let instructions_hash = HashOf::new(&instructions);
    ...
    let err = test_client
        ...
        .execute_single()
        .expect_err("domain shouldn't be created without enough approvals");
    assert!(matches!(err, SingleQueryError::ExpectedOneGotNone));
    ...
    // Check that the domain has been created and is owned by the root multisig account
