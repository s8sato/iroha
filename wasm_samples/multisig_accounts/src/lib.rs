...
fn main(_id: TriggerId, owner: AccountId, event: EventBox) {
    ...
    let can_execute_multisig_transactions_registry = CanExecuteUserTrigger {
        trigger: multisig_transactions_registry_id,
    };
    ...
    Revoke::account_role(role_id.clone(), owner)
        .execute()
        .dbg_expect("accounts registry should successfully revoke the multisig role from the registry owner");
}
